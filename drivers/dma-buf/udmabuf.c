...
udmabuf_export(struct udmabuf_args args, int *fd, struct thread *td)
{
	...
	udmabuf_args_check(args, &nr_pages);
    ...
    struct thread *td)
{
	...
	/* TODO: may be we can modify the define of udmabuf_create_list to use pointer instead of flexible array
	    to avoid this kind of ugly code? */
	/* copy the flexible array of udmabuf_create_list in userspace */
	struct ioctl_args *uap = (struct ioctl_args *)td->td_sa.args;
	caddr_t uptr = uap->data;
...
udmabuf_load(void)
{
	...
	// TODO: use miscdevice_create (linuxkpi) 
	printf("udmabuf load\n");
